...
class DataGraph:
    def __init__(self, data):
        self.data = data
        self.graph = graphviz.Digraph(comment="databank", format="png", graph_attr={'rankdir': 'LR', 'dpi': '300'})
        self.create_graph()
    ...
    def create_graph(self):
        ...
        if True:
            ...
            for data_item in unprocessed_data:
                # Add unprocessed data nodes (2nd nodes)
                unprocessed_data_node_name = f"{device_name}_{data_item.replace(' ', '_')}_UnprocessedData"
                self.graph.node(unprocessed_data_node_name, label=data_item, style='filled', fillcolor='lightgreen', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
                
                # Connect device node with unprocessed data nodes
                self.graph.edge(device_name, unprocessed_data_node_name, dir="none", penwidth="3")
                ...
                self.add_sensitivity_node(device_name, sensitivity_level, data_item, sensitivity_action, sensitivity_frequency)

                # Add action nodes
                self.add_action_node(device_name, sensitivity_action, data_item)

                # Connect unprocessed data nodes with sensitivity nodes
                self.graph.edge(unprocessed_data_node_name, f"{device_name}_{data_item}_Sensitivity", dir="none", penwidth="3")
                ...
                self.add_combined_node(device_name, sensitivity_level, sensitivity_action, data_item, sensitivity_action, sensitivity_frequency)

                # Add edge between action node and combined node
                self.graph.edge(f"{device_name}_{data_item}_{sensitivity_action}_Action", combined_node_name, dir="none", penwidth="3")
                ...
                self.graph.edge(f"{device_name}_{data_item}_{sensitivity_action}_Independent", combined_node_name, dir="none", penwidth="3")
                ...
                if "cate_service" in entry:
                    ...
                    if data_item in cate_service:
                        ...
                        category = service_data["category"]  # Retrieve category from service_data
                        frequency = service_data["frequency"]  # Retrieve frequency from service_data
                        # Add the cate_service node
                        cate_service_node_name = f"{device_name}_{data_item}_{action}_CateService"
                        self.graph.node(cate_service_node_name, label=action, style='filled', fillcolor='lightblue', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
                        # Connect the cate_service node with the corresponding combined node
                        self.graph.edge(combined_node_name, cate_service_node_name, dir="none", penwidth="3")

                        # Add the 7th node (category node)
                        category_node_name = self.add_category_node(device_name, data_item, action, frequency, category)
                        # Connect the category node with the cate_service node
                        self.graph.edge(cate_service_node_name, category_node_name, dir="none", penwidth="3")

                        # Add the 8th node (service_type node)
                        service_type = entry["service"]["service_type"]
                        service_type_node_name = self.add_service_type_node(device_name, data_item, action, service_type)
                        # Connect the service_type node with the category node
                        self.graph.edge(category_node_name, service_type_node_name, dir="none", penwidth="3")
                        
                        # Add the 9th node (service_name node)
                        service_name = entry["service"]["service_name"]
                        service_name_node_name = self.add_service_name_node(device_name, data_item, action, service_name)
                        # Connect the service_name node with the service_type node
                        self.graph.edge(service_type_node_name, service_name_node_name, dir="none", penwidth="3")

    # Add device node (1st node)
    def add_device_node(self, device_name, device_type):
        label = f"{device_type}\n{device_name}"
        self.graph.node(device_name, label=label , style='filled', fillcolor='lightgreen', fontsize='24', fontcolor='black',fontname="bold",shape = "circle")
    ...
    def add_unprocessed_data_nodes(self, device_name, unprocessed_data):
        for data_item in unprocessed_data:
            ...
            self.graph.node(node_name, label=node_label, style='filled', fillcolor='lightgreen', fontsize='24', fontcolor='black', fontname="bold", shape="circle")

    # Add sensitivity nodes (3rd nodes)
    def add_sensitivity_node(self, device_name, sensitivity_level, data_item, sensitivity_action, sensitivity_frequency):
        node_name = f"{device_name}_{data_item}_Sensitivity"
        # Call determine_category method to get the category
        category = self.determine_category(sensitivity_action, sensitivity_frequency, sensitivity_level)
    ...
    def add_action_node(self, device_name, action, data_item):
        ...
        self.graph.node(node_name, label=f"{action}", style='filled', fillcolor='lightgreen', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
        # Connect action nodes with sensitivity nodes
        self.graph.edge(f"{device_name}_{data_item}_Sensitivity", node_name, dir="none", penwidth="3")

    # Add combined nodes (5th nodes)
    def add_combined_node(self, device_name, sensitivity_level, raw_data_action, data_item, sensitivity_action, sensitivity_frequency):
        # Determine category based on sensitivity level
        category = self.determine_category(sensitivity_action, sensitivity_frequency, sensitivity_level)
        node_name = f"{device_name}_{data_item}_{sensitivity_level}_{raw_data_action}_Combined"
        self.graph.node(node_name, label=f"{category}\n{raw_data_action}", style='filled', fillcolor='orange', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
    ...
    def add_independent_node(self, device_name, data_item, sensitivity_action):
        ...
        self.graph.node(node_name, label=node_label, style='filled', fillcolor='orange', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
    ...
    def add_service_type_node(self, device_name, data_item, action, service_type):
        ...
        self.graph.node(node_name, label=node_label, style='filled', fillcolor='lightblue', fontsize='24', fontcolor='black', fontname="bold", shape="circle")
        return node_name
